...
- 노드(node), 루트(root) 노드,
- 부모 노드, 자식 노드, 형제(sibling) 노드, 조상(ancestor) 노드,
후손(descendent) 노드
...
- 완전 이진 트리(complete binary tree): 마지막 레벨 외에는 모두 다 차있고 마지막
레벨은 왼쪽부터 차있는 트리 (중간 빈곳 X)
*/
...
int main() {
  ...
  Node* n1 =
      new Node{1, nullptr, nullptr};  // 물결괄호 초기값 나열 (생성자 아님)
